...
void Physics::Step(float dt)
{
	...
	{
		if ((pow(((app->player->ovni->position.x + (app->player->playerRect.w / 2)) - app->scene->planetList[i]->position.x), 2) + pow(((app->player->ovni->position.y + (app->player->playerRect.h / 2)) - app->scene->planetList[i]->position.y), 2)) < (app->scene->planetList[i]->fluidRad ^ 2))
		{
			if (app->scene->planetList[i]->id == 6)
			{
				app->player->ovni->forces.Add(app->player->ovni->ForceHydroDrag(app->player->ovni->velocity));
				app->player->ovni->forces.Add(app->player->ovni->ForceHydroBuoy(app->scene->planetList[i]->density, app->scene->planetList[i]->gravity, app->player->playerRect));
			}
			else
			{
		...
		if ((pow(((app->player->ovni->position.x + (app->player->playerRect.w / 2)) - app->scene->planetList[i]->position.x), 2) + pow(((app->player->ovni->position.y + (app->player->playerRect.h / 2)) - app->scene->planetList[i]->position.y), 2)) < (app->scene->planetList[i]->orbitRad ^ 2))
		{
...
fPoint Spaceship::ForceHydroBuoy(float density, fPoint gravity, SDL_Rect playerRect)
{
...
Spaceship::Spaceship(fPoint position, float mass, CircleCollider* collider, fPoint acceleration, float frontalArea, int fuel)
{
	this->position = position;
	this->mass = mass;
	this->collider = collider;
...
void Spaceship::ApplyForce(Spaceship body, fPoint force)
{
